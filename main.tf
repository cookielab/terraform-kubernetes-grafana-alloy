locals {
  ...
  url_metrics_write = var.metrics.backend_type == "mimir" ? format("https://%s/api/v1/push", trimsuffix(var.metrics.endpoint, "/")) : format("https://%s/api/v1/write", trimsuffix(var.metrics.endpoint, "/"))
